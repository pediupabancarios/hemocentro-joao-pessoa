...
create table if not exists receitas (
  ...
  medicamento text not null,   -- resumo (ex.: "Med A, Med B") — ver itens_json p/ lista completa
  uso text,
  via text,             -- Oral / EV / IM / SC / Inalatório
  itens_json text,      -- JSON [{medicamento, uso}] — permite vários medicamentos por receita
  prescricao text,
...
alter table receitas add column if not exists itens_json text;
...
create table if not exists solicitacoes_exames (
  ...
  medico text default 'Dr. Rodrigo Cahuana',
  medico_crm text,      -- aparece na "assinatura" do documento impresso
...
alter table solicitacoes_exames add column if not exists medico_crm text;
...
alter table solicitacoes_hemocomponentes add column if not exists medico_crm text;

-- ==========================================================
-- 4b. Ficha de notificação/investigação de incidentes transfusionais (F.CQ.403)
-- ==========================================================
create table if not exists fichas_incidente_transfusional (
  id uuid primary key default gen_random_uuid(),
  paciente_id uuid references pacientes(id) on delete cascade,
  paciente_nome text,
  numero text,
  ...
  sinais_sintomas text,            -- lista separada por "; "
  gravidade text,                  -- Grau I..IV
  data_ocorrencia date,
  estabelecimento text default 'HEMOCENTRO DA PARAIBA',
  cnes text default '2399598',
  tipo_transfusao text,            -- Alogênica / Autóloga
  indicacao_transfusao text,
  setor text,
  data_transfusao date,
  ocupacao text,
  hemocomponentes text,             -- legado (texto livre) — ver hemocomponentes_json
  hemocomponentes_json text,        -- JSON [{tipo,num,qualif,abo,instituicao,cnes}]
  tipo_reacao text,                -- Imediata / Tardia
  reacoes_imediatas text,          -- lista separada por "; "
  reacoes_tardias text,            -- lista separada por "; "
  responsavel text,
  medico text,
  created_at timestamptz default now()
);
alter table fichas_incidente_transfusional add column if not exists ocupacao text;
alter table fichas_incidente_transfusional add column if not exists hemocomponentes_json text;

-- ==========================================================
-- 4c. Solicitação de Medicamentos — Componente Especializado
--     da Assistência Farmacêutica (LME / CEAF)
-- ==========================================================
create table if not exists solicitacoes_medicamento_ceaf (
  id uuid primary key default gen_random_uuid(),
  paciente_id uuid references pacientes(id) on delete cascade,
  paciente_nome text,
  data date default current_date,
  ...
  peso text,
  altura text,
  medicamentos_json text,          -- JSON [{nome, m1, m2, m3, m4, m5, m6}]
  cid10 text,
  diagnostico text,
  anamnese text,
  tratamento_previo text,          -- vazio = NÃO; texto = SIM (relato)
  incapaz text,                    -- vazio = capaz; texto = nome do responsável
  medico text,
  cns_medico text,
  preenchido_por text,             -- Paciente / Mãe / Responsável / Médico / Outro
  preenchido_cpf text,
  telefone text,
  doc_tipo text,                   -- CPF / CNS
  doc_numero text,
  email text,
  created_at timestamptz default now()
);
...
alter table fichas_incidente_transfusional enable row level security;
alter table solicitacoes_medicamento_ceaf  enable row level security;
...
drop policy if exists "allow all - fichas_incidente"   on fichas_incidente_transfusional;
drop policy if exists "allow all - med_ceaf"           on solicitacoes_medicamento_ceaf;
...
create policy "allow all - fichas_incidente" on fichas_incidente_transfusional for all using (true) with check (true);
create policy "allow all - med_ceaf"         on solicitacoes_medicamento_ceaf  for all using (true) with check (true);
